...
class UniSRec(SASRec):
    ...
    def cand_sort_predict(self, plm_embedding, evaluate_data, k=15):
        '''
        sort the items in the candidate set
        '''
        preds, last_item = torch.tensor([]), torch.tensor([])
        plm_embedding = plm_embedding.to(self.device)
        for _, (seq, seq_length, target, cand) in enumerate(tqdm(evaluate_data)):
            item_seq_len = seq_length.to(self.device)
            item_emb_list = self.moe_adaptor(plm_embedding(seq.to(self.device)))
            seq_output = self.forward(seq.to(self.device), item_emb_list, item_seq_len)
            ...
            scores = torch.gather(scores, 1, cand.to(self.device)).topk(k)[1]
            rank_list = torch.gather(cand.to(self.device), 1, scores)
